...
SuperpoweredExample::SuperpoweredExample(unsigned int samplerate, unsigned int buffersize, const char *path, int fileAoffset, int fileAlength, int fileBoffset, int fileBlength) : activeFx(0), crossValue(0.0f), volB(0.0f), volA(1.0f * headroom) {
    ...
    whitenoiseBuffer = (float *)memalign(16, (buffersize + 16) * sizeof(float) * 2);
...
void SuperpoweredExample::onPlayPause(bool play, bool buttonPressed) {
    if (!play) {
        if(testFile) {
            if(!buttonPressed) {
                if (testFile != NULL) {
                    //fwrite(bufferBuffer, 1, sizeOfBuffer, testFile);
                    fclose(testFile);
                    testFile = NULL;
                    //bufferBuffer = NULL;
                }
...
bool SuperpoweredExample::process(short int *output, unsigned int numberOfSamples) {
    ...
    */
            __android_log_print(ANDROID_LOG_VERBOSE, "SuperpoweredExample", "NSamples %i", numberOfSamples);
        ...
        if (fifoOutputLastSample - fifoOutputFirstSample >= numberOfSamples) {
            whitenoiseBuffer = (fifoOutput + fifoOutputFirstSample * 2);
    ...
    if (!silence) {
        sizeOfBuffer = sizeOfBuffer + sizeof(whitenoiseBuffer);
        if (testFile) {
            fwrite(whitenoiseBuffer, sizeOfBuffer, 1, testFile);
        }
        SuperpoweredFloatToShortInt(whitenoiseBuffer, output, numberOfSamples);


        __android_log_print(ANDROID_LOG_VERBOSE, "SuperpoweredExample", "OutputSamples %zu", sizeof(output));
        //sizeOfBuffer = sizeOfBuffer + sizeof(output);
        //bufferBuffer += (int)*output;



        /*sizeOfBuffer = sizeOfBuffer + sizeof(stereoBuffer);
        if(testFile) {
            fwrite(output, 1, sizeOfBuffer, testFile);
        }*/
    }
    return !silence;
}
